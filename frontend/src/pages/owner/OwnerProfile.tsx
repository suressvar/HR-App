...
export const OwnerProfile: React.FC = () => {
  ...
  // GET /api/profile
  const { data: profile, isLoading, isError, error } = useQuery<OwnerProfileData>({
    queryKey: ['ownerProfile'],
    queryFn: () => fetchApi<OwnerProfileData>('/profile', {}, token),
  });

  // Pre-fill form fields when profile is loaded
  useEffect(() => {
    if (profile) {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: (updatedData: { name: string; email: string; phone: string; personalInfo: string }) =>
      fetchApi<OwnerProfileData>('/profile', { method: 'PATCH', body: JSON.stringify(updatedData) }, token),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ownerProfile'] });
      queryClient.invalidateQueries({ queryKey: ['authMe'] });
      setToastMessage('Owner profile updated successfully.');
      setFormError(null);
    },
    onError: (err: any) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setFormError(null);
    updateProfileMutation.mutate({ name, email, phone, personalInfo });
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-ink tracking-tight">Executive Profile</h1>
        <p className="text-ink-muted text-sm mt-1">Manage your account information and contact details.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-border p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border">
          ...
          <div>
            ...
            <p className="text-xs text-ink-muted font-medium">System Owner & Executive Administrator</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-ink-muted uppercase mb-1">Personal Info / Bio</label>
            <textarea
              rows={3}
              value={personalInfo}
              onChange={(e) => setPersonalInfo(e.target.value)}
              placeholder="Additional contact preferences or executive notes..."
